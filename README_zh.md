...
`ecosystem/examples/` 是 Airymax AI Agent 运行时平台的**官方示例 Agent 集合**。每个示例都是自包含、可运行的项目，含独立的 `README.md`、`config.yaml` 与 `*.agent.yaml` 定义，演示 Airymax SDK 与 AgentRT 运行时的一项具体能力。

整个集合按难度分级组织：每个示例声明难度（入门 / 中级 / 高级）及所展示的 Airymax 能力，开发者可按需选择入口并按顺序进阶。
...
| [`weather-agent`](weather-agent/) | 入门 | 自定义工具 + MCP 协议集成 |
| [`plugin-demo`](plugin-demo/) | 入门 | Plugin SDK 四型插件（Agent / Tool / Hook / Skill） |
| [`code-review-agent`](code-review-agent/) | 中级 | 代码审查 Skill + 安全扫描 |
| [`prompt-tuner-demo`](prompt-tuner-demo/) | 中级 | 提示词调优框架使用 + 评估数据集 |
...
| [`multi-agent-debate`](multi-agent-debate/) | 高级 | 4 种多 Agent 协作模式（Sequential / Parallel / Debate / Hierarchical） |
| [`a2a-chat`](a2a-chat/) | 高级 | A2A（Agent-to-Agent）协议 Agent 间通信 |
